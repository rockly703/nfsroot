...
#define ANON_PRINT(fmt, ...) printk("anon_inodefs print:"fmt, ##__VA_ARGS__)

#define DEBUGFS_ERROR(fmt, ...) printk("debugfs error:"fmt, ##__VA_ARGS__)
#define DEBUGFS_INFO(fmt, ...) printk("debugfs info:"fmt, ##__VA_ARGS__)
#define DEBUGFS_PRINT(fmt, ...) printk("debugfs print:"fmt, ##__VA_ARGS__)
...
void debugfs_tranverse(struct dentry *root) 
{
    ...
    if (!root) {
        DEBUGFS_PRINT("directry is NULL.\n"); 
        return;
    } 
...
resume:
    while (dir != &parent->d_subdirs) {
        ...
        DEBUGFS_PRINT("directory name is %s\n", dentry->d_name.name);
    }
